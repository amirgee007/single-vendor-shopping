...
class Payumoney extends Model
{
 ...
 //For Fetch Coupon details
    ...
    public static function get_related_deals($id)
    {
        ...
        return DB::table('nm_deals')->where('deal_status', '=', 1)->where('deal_end_date', '>', $date)->where('deal_id', '!=', $id)->where('deal_main_category', '=', $catid)->LeftJoin('nm_maincategory', 'nm_maincategory.mc_id', '=', 'nm_deals.deal_category')->LeftJoin('nm_secmaincategory', 'nm_secmaincategory.smc_id', '=', 'nm_deals.deal_main_category')->LeftJoin('nm_subcategory', 'nm_subcategory.sb_id', '=', 'nm_deals.deal_sub_category')->LeftJoin('nm_secsubcategory', 'nm_secsubcategory.ssb_id', '=', 'nm_deals.deal_second_sub_category')->take(3)->get();
    }
    ...
    public static function getorderdetails($id)
    {
        ...
        return DB::select(DB::raw(" select * from nm_order_payu o left join nm_product p on o.order_pro_id=p.pro_id and o.order_type=1 left join nm_deals d on o.order_pro_id=d.deal_id and o.order_type=2 where o.order_id in ($id)"));
    ...
    public static function get_store_list()
    {
      /*Here Merchant's Only one store will be displayed in front end- other stores will show in store detail ->branchs*/
        return DB::table('nm_store')
        ->select('stor_img','stor_name','stor_name_fr','stor_id')
        ->join('nm_merchant','nm_merchant.mer_id','=','nm_store.stor_merchant_id')
        ->where('nm_merchant.mer_staus','=',1)
        ->where('nm_store.stor_status', '=', 1)
...
You have now ' . $qty . ' Product(s) in your cart.';
    ...
    {
        return DB::table('nm_order_payu')->insert($data);
    }
    ...
    public static function paypal_checkout_update($data, $insert_id)
    {
        ...
        for ($i = 0; $i < $insert_id_count; $i++) {
            $result         = DB::table('nm_order_payu')->where('order_id', '=', $insert_id_exp[$i])->update($data);
            $order_pro_type = DB::table('nm_order_payu')->where('order_id', $insert_id_exp[$i])->value('order_type');
            $order_pro_id   = DB::table('nm_order_payu')->where('order_id', $insert_id_exp[$i])->value('order_pro_id');
            $order_pro_qty  = DB::table('nm_order_payu')->where('order_id', $insert_id_exp[$i])->value('order_qty'); 
            if ($order_pro_type == 1) {
                #product    
    ...
    {
        return DB::table('nm_order_payu')->select('transaction_id','order_merchant_id','order_pro_id','order_type')->where('transaction_id','=',$trans_id)->groupBy('order_merchant_id')->get();
    }

    /* Merchant mail subtotal calculation paypal order */
    public static function get_PayPalOrd_mer_subtotal($transid,$merchant_id)
    {
        return DB::table('nm_order_payu')->where('transaction_id', '=', $transid)->where('order_merchant_id','=',$merchant_id)->sum('order_amt');
        
    }
    /* Merchant mail tax calculation paypal order*/
    public static function get_PayPalOrd_mer_tax($transid,$merchant_id)
    {
        return DB::table('nm_order_payu')->where('transaction_id', '=', $transid)->where('order_merchant_id','=',$merchant_id)->sum('order_tax');
    }
    ...
    {
        return DB::table('nm_order_payu')->where('transaction_id', '=', $transid)->where('order_merchant_id','=',$merchant_id)
    ...
    /* get tax details for payumoney */
    public static function order_tax_details($id)
    {
        return DB::table('nm_order_payu')->select('tax_id_number','tax_type')->where('order_id', '=', $id)->first();
    }
    ...
    public static function getAll_Ordtax_details($mer_id,$trans_id){
        ...
        return DB::table('nm_order_payu AS ord1')
        ...
        return DB::table('nm_order_payu AS ord1')
            ->select(
                DB::raw("SUM(ord1.order_amt) as sumOrdAmt,
                   (SELECT SUM(ord2.order_tax)  FROM nm_order_payu AS ord2 WHERE ord1.order_merchant_id = ord2.order_merchant_id) AS sumTax,
                   (SELECT SUM(ord3.order_shipping_amt) FROM nm_order_payu AS ord3 WHERE ord1.order_merchant_id = ord3.order_merchant_id) AS sumShippingAmt,
                   (SELECT SUM(ord4.coupon_amount) FROM nm_order_payu AS ord4 WHERE ord1.order_merchant_id = ord4.order_merchant_id) AS sumCoupon,
                   (SELECT SUM(ord5.wallet_amount) FROM nm_order_payu AS ord5 WHERE ord1.order_merchant_id = ord5.order_merchant_id) AS sumWallet,
                   (SELECT SUM(ord6.mer_commission_amt) FROM nm_order_payu AS ord6 WHERE ord1.order_merchant_id = ord6.order_merchant_id) AS sumMerchantCommission,
                   (SELECT SUM(ord7.mer_amt) FROM nm_order_payu AS ord7 WHERE ord1.order_merchant_id = ord7.order_merchant_id) AS sumMerchantAmount  
                   "
                )
            )
    ...
    public static function sumOfOrderAmount_single($mer_id,$trans_id){
        ...
        return DB::table('nm_order_payu AS ord1')
            ->select(
                DB::raw("SUM(ord1.order_amt) as sumOrdAmt,
                   (SELECT SUM(ord2.order_tax)  FROM nm_order_payu AS ord2 WHERE ord1.order_merchant_id = ord2.order_merchant_id and ord1.transaction_id = ord2.transaction_id ) AS sumTax,
                   (SELECT SUM(ord3.order_shipping_amt) FROM nm_order_payu AS ord3 WHERE ord1.order_merchant_id = ord3.order_merchant_id  and ord1.transaction_id = ord3.transaction_id) AS sumShippingAmt,
                   (SELECT SUM(ord4.coupon_amount) FROM nm_order_payu AS ord4 WHERE ord1.order_merchant_id = ord4.order_merchant_id  and ord1.transaction_id = ord4.transaction_id) AS sumCoupon,
                   (SELECT SUM(ord5.wallet_amount) FROM nm_order_payu AS ord5 WHERE ord1.order_merchant_id = ord5.order_merchant_id  and ord1.transaction_id = ord5.transaction_id) AS sumWallet,
                   (SELECT SUM(ord6.mer_commission_amt) FROM nm_order_payu AS ord6 WHERE ord1.order_merchant_id = ord6.order_merchant_id  and ord1.transaction_id = ord6.transaction_id) AS sumMerchantCommission,
                   (SELECT SUM(ord7.mer_amt) FROM nm_order_payu AS ord7 WHERE ord1.order_merchant_id = ord7.order_merchant_id  and ord1.transaction_id = ord7.transaction_id) AS sumMerchantAmount  
                   "
                )
            )
        ->where('ord1.transaction_id', '=', $trans_id)->where('ord1.order_merchant_id', '=', $mer_id)->first();
