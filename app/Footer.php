...
class Footer extends Model
{
    ...
    {
        return DB::table('nm_faq')->where('faq_status', 0)->get();
    }

    public static function get_help_details()
    {
        return DB::table('nm_cms_pages')->where('cp_title','Help')->where('cp_status', 1)->get();
    }
    ...
    public static function get_blog_cus_cmt($id)
    {
        ...
        foreach ($check_approve as $admin_approve) {
            $adm_status = $admin_approve->bs_radminapproval;
            $adm_postpage = $admin_approve->bs_postsppage;
        }
        if ($adm_status == 1) {
            return DB::table('nm_blog_cus_comments')->where('cmt_blog_id', '=', $id)->where('cmt_admin_approve', '=', 1)->paginate($adm_postpage);
        } else {
            return DB::table('nm_blog_cus_comments')->where('cmt_blog_id', '=', $id)->paginate($adm_postpage);
        }
    }
