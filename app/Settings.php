...
class Settings extends Model
{
    ...
    public static function get_noimage_details()
    {
       // return DB::table('nm_imagesetting')->where('imgs_type', '=', 3)->get();
        return DB::table('nm_imagesetting')->get();
    }
	
	 public static function get_noimage_details_dynamic($type)
    {
        return DB::table('nm_imagesetting')->where('imgs_type', '=', $type)->get();
    }
    ...
    public static function update_noimage($filename,$types)
    {
        return DB::table('nm_imagesetting')->where('imgs_type', '=', $types)->update(array(
            'imgs_name' => $filename
        ));
    }
    ...
    {
        return DB::table('nm_newsletter_subscribers')->where('status','!=','2')->get();
    }
	...
	public static function manage_language(){
		return DB::table('nm_language')->get();
	}
	
	public static function update_default_lang_submit($id, $entry){
        return DB::table('nm_language')->where('lang_id', '=', $id)->update($entry);
        
    }

    public static function update_default_lang_submit1($entry){
        return DB::table('nm_language')->update($entry);
        
    }
	public static function status_lang($id, $status)
    {
        return DB::table('nm_language')->where('lang_id', '=', $id)->update(array('lang_status' => $status));
