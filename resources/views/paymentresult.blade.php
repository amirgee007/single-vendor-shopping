...
<html lang="en">
 <head>
    ...
    <link href="{{ url('') }}/plug-k/demo/css/demo.css" rel="stylesheet">
<!-- Bootstrap style --> 
  ...
  <h5> <?php if (Lang::has(Session::get('lang_file').'.PRODUCT_DETAILS_FOR_CURRENT_TRANSACTION')!= '') { echo  trans(Session::get('lang_file').'.PRODUCT_DETAILS_FOR_CURRENT_TRANSACTION');}  else { echo trans($OUR_LANGUAGE.'.PRODUCT_DETAILS_FOR_CURRENT_TRANSACTION');} ?> </h5>	
          ...
          ?>
                ...
                ?>
                    ...
                    <tr>               	  
                        <td>&nbsp;</td>                  
                        <td style="font-weight:bold;">{{ (Lang::has(Session::get('lang_file').'.TOTAL')!= '') ? trans(Session::get('lang_file').'.TOTAL') : trans($OUR_LANGUAGE.'.TOTAL') }}</td>                  
	...
	<h4>		
	 <a href="<?php echo url('index'); ?>" class="btn btn-large pull-right me_btn res-cont1"><span style="font-weight:normal;" >{{ (Lang::has(Session::get('lang_file').'.CONTINUE_SHOPPING')!= '') ? trans(Session::get('lang_file').'.CONTINUE_SHOPPING') : trans($OUR_LANGUAGE.'.CONTINUE_SHOPPING') }}</span> <i class="icon-arrow-right"></i>  </a> 	
	</h4>
</div>
</div></div>
</div>
