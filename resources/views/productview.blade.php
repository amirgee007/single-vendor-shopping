...
.nbs-flexisel-nav-right.disabled {
    opacity: 0.4;
      ...
      <title>{{ $pro_details_by_id->$pro_title }}</title>
     <meta name="viewport" content="width=device-width, initial-scale=1.0">
     <meta name="description" content="{{ $pro_details_by_id->$pro_mdesc  }}">
     <meta name="keywords" content="{{ $pro_details_by_id->$pro_mkeywords  }}">
     <meta name="author" content="{{ $pro_details_by_id->$pro_mkeywords  }}">
@endif
   </head>
   <body>
      ...
      <div class="container">
		...
		@php	$product_img= explode('/**/',trim($pro_details_by_id->pro_Img,"/**/"));	
         ...
         <a id="Zoomer3"  href="{{ $prod_path }}" class="MagicZoomPlus" rel="selectors-effect: fade; selectors-change: mouseover; " title="{{ $alt_text }}"><img src="{{ $prod_path }}" alt="{{ $alt_text }}" /></a> <br/>
 ...
 @for($i=0;$i < $img_count;$i++)  
      @php  $product_image     = $product_img[$i];
       ...
       @php $alt_text   = substr($pro_details_by_id->$pro_title,0,25);
        ...
        <a href="{{ $prod_path }}" rel="zoom-id: Zoomer3" rev="{{ $prod_path }}"><img src="{{ $prod_path }}" alt="{{ $alt_text }}" style="width:87px; height:100px;"/></a>
 @endfor</div>
            ...
            <div class="pro-right">
               <div class="pro-rightop">
				  ...
				  @if((Session::get('lang_code')) == '' || (Session::get('lang_code'))== 'en')  
					@php	$pro_title = 'pro_title'; @endphp
				   @else @php  $pro_title = 'pro_title_'.Session::get('lang_code'); @endphp @endif
				  {{ $pro_details_by_id->$pro_title }} </h2>
				  @php
					...
					@else
						@if (Lang::has(Session::get('lang_file').'.NO_RATING_FOR_THIS_PRODUCT')!= '') {{  trans(Session::get('lang_file').'.NO_RATING_FOR_THIS_PRODUCT') }} @else {{ trans($OUR_LANGUAGE.'.NO_RATING_FOR_THIS_PRODUCT') }} @endif
					@endif
					<span class="Review-count"><strong>( {{ $count_review_rating }} @if (Lang::has(Session::get('lang_file').'.REVIEWS_AND_RATINGS')!= '') {{  trans(Session::get('lang_file').'.REVIEWS_AND_RATINGS') }} @else {{ trans($OUR_LANGUAGE.'.REVIEWS_AND_RATINGS') }} @endif )</strong></span>
					</p>
					...
					@if(($new_start_date <= $current_date) && ($new_end_date >= $current_date))
						...
						@else
							@if(Session::has('customerid'))
					 		...
					 		@endif 
							@else 
								<a href="#login" role="button" data-toggle="modal" style="padding-right:0; margin-top:5px;"><button class="wish-but" id="login_a_click"  value="{{ (Lang::has(Session::get('lang_file').'.LOGIN')!= '') ? trans(Session::get('lang_file').'.LOGIN') : trans($OUR_LANGUAGE.'.LOGIN') }}"> {{ (Lang::has(Session::get('lang_file').'.CLICK_TO_VIEW_COUPON_CODE')!= '') ?  trans(Session::get('lang_file').'.CLICK_TO_VIEW_COUPON_CODE') : trans($OUR_LANGUAGE.'.CLICK_TO_VIEW_COUPON_CODE') }} </button></a>
                ...
                <div class="pro-mid">
					...
					@if($count > 0) 
						<div class="detail-list">
							...
							@if (Lang::has(Session::get('lang_file').'.QUANTITY')!= '') {{ trans(Session::get('lang_file').'.QUANTITY') }} @else {{ trans($OUR_LANGUAGE.'.QUANTITY') }} @endif :</label>
							</div>
							<div class="details-left-select">
								<div class="handle-counter" id="handleCounter">
									<a class="counter-minus btn btn-danger" onClick="remove_quantity()">-</a>
									<input type="number" value="1" min="1" max="{{  ($pro_details_by_id->pro_qty - $pro_details_by_id->pro_no_of_purchase) }}" name="addtocart_qty" id="addtocart_qty" readonly required >
						...
						<div class="detail-list">
							<div class="details-left-label">
								<label class="left-label">@if (Lang::has(Session::get('lang_file').'.COLOR')!= '') {{ trans(Session::get('lang_file').'.COLOR') }} @else {{ trans($OUR_LANGUAGE.'.COLOR') }} @endif : </label>
							</div>
							<div class="details-left-select">
						...
						<div class="detail-list">
							<div class="details-left-label">
								<label class="left-label">@if (Lang::has(Session::get('lang_file').'.SIZE')!= '') {{ trans(Session::get('lang_file').'.SIZE') }} @else {{ trans($OUR_LANGUAGE.'.SIZE') }} @endif :</label>
							</div>
							<div class="details-left-select">
					...
					@endif @endif 
						...
						<div class="detail-list">
							<div class="details-left-label">
								<label class="left-label"> 
							 @if (Lang::has(Session::get('lang_file').'.AVAILABLE_STOCK')!= '') {{ trans(Session::get('lang_file').'.AVAILABLE_STOCK') }} @else {{ trans($OUR_LANGUAGE.'.AVAILABLE_STOCK') }} @endif :</label>
							</div>
							<div class="details-left-select">
							   <p class="stock-list">{{ $pro_details_by_id->pro_no_of_purchase }}  
							  @if (Lang::has(Session::get('lang_file').'.SOLD')!= '') {{ trans(Session::get('lang_file').'.SOLD') }} @else {{ trans($OUR_LANGUAGE.'.SOLD') }} @endif / {{ $pro_details_by_id->pro_qty-$pro_details_by_id->pro_no_of_purchase }} 
							  @if (Lang::has(Session::get('lang_file').'.IN_STOCK')!= '') {{  trans(Session::get('lang_file').'.IN_STOCK') }} @else {{ trans($OUR_LANGUAGE.'.IN_STOCK') }} @endif</p>
							</div>
						</div>
				...
				<input type="hidden" name="return_url" value="<?php echo $pro_details_by_id->mc_name.'/'.base64_encode(base64_encode(base64_encode($pro_details_by_id->pro_mc_id))); ?>" />
					...
					@if(Session::has('customerid'))
                        ...
                        {{ Form::hidden('pro_id','$pro_details_by_id->pro_id') }}        
    							...
    							<button type="button" onclick="addtowish({{ $pro_details_by_id->pro_id }},{{ Session::get('customerid') }})">
    							<input type="hidden" id="wishlisturl" value="{{ url('user_profile?id=4') }}">
    								<i class="fa fa-heart" aria-hidden="true"></i>@if (Lang::has(Session::get('lang_file').'.ADD_TO_WISHLIST')!= '') {{ trans(Session::get('lang_file').'.ADD_TO_WISHLIST') }} @else {{ trans($OUR_LANGUAGE.'.ADD_TO_WISHLIST') }} @endif 
    							</button>
                            @else
                            <?php /* remove wishlist */?>   
                                
                            <a href="{!! url('remove_wish_product').'/'.$prodInWishlist->ws_id!!}">
                                <button type="button">@if (Lang::has(Session::get('lang_file').'.REMOVE_FROM_WISHLIST')!= '') {{  trans(Session::get('lang_file').'.REMOVE_FROM_WISHLIST') }} @else {{ trans($OUR_LANGUAGE.'.REMOVE_FROM_WISHLIST') }} @endif</button>
                            </a> 
        ...
        @if($pro_details_by_id->$pro_desc != '')
         <div class="pro-description">
            ...
            <p>{!! html_entity_decode($pro_details_by_id->$pro_desc) !!}</p>
            <?php /* <p>{{ $pro_details_by_id->$pro_desc }}</p> */ ?>   
         </div>
        @endif 
         @if(count($product_specGroupList)>0)
             <div class="pro-features">
                <h5 class="pro-description-title">@if (Lang::has(Session::get('lang_file').'.PRODUCT_SPECIFICATION')!= '') {{ trans(Session::get('lang_file').'.PRODUCT_SPECIFICATION') }} @else {{ trans($OUR_LANGUAGE.'.PRODUCT_SPECIFICATION') }} @endif</h5>
                                    ...
                                    @if($prodSpecAr[$prodSpec->sp_id])  
                                        ...
                                        <div class="spec-top">
                                            <div class="spec-label">
                                                <label>{{ $prodSpec->$sp_name }}</label>
                                            </div>
                                            <div class="spec-right">
                                                @php $i=1; @endphp
                                                @foreach($prodSpecAr[$prodSpec->sp_id] as $spcVal)

                                                    <p class="">{{ $spcVal->$spc_value }} 
		...
		@endif 
         <div class="Ratings-review">
            <div class="review-title">
               <h5 class="">@if (Lang::has(Session::get('lang_file').'.REVIEWS_AND_RATINGS')!= '') {{ trans(Session::get('lang_file').'.REVIEWS_AND_RATINGS') }} @else {{ trans($OUR_LANGUAGE.'.REVIEWS_AND_RATINGS') }} @endif</h5>
			   ...
			   <a href="#login" role="button" data-toggle="modal"  id="login_a_click"  value="@if (Lang::has(Session::get('lang_file').'.LOGIN')!= '') {{  trans(Session::get('lang_file').'.LOGIN') }} @else {{ trans($OUR_LANGUAGE.'.LOGIN') }} @endif">@if (Lang::has(Session::get('lang_file').'.WRITE_A_REVIEW')!= '') {{  trans(Session::get('lang_file').'.WRITE_A_REVIEW') }} @else {{ trans($OUR_LANGUAGE.'.WRITE_A_REVIEW') }} @endif</a>
     ...
     @php	$r=0; @endphp
		...
		@elseif($customer_ratings=='5')
		 ...
		 @endif
                  ...
                  <h5>{{ $review_title }}</h5>
                  <h6>by {{ $customer_name }} on {{ $final_date }}</h6>
                  <p>{{ $customer_comments }}.</p>
               </div>
            </div>
       <!-- //foreach --> @endforeach
			@if($r==0) @if (Lang::has(Session::get('lang_file').'.NO_RATING_FOR_THIS_PRODUCT')!= '') {{ trans(Session::get('lang_file').'.NO_RATING_FOR_THIS_PRODUCT') }} @else {{ trans($OUR_LANGUAGE.'.NO_RATING_FOR_THIS_PRODUCT') }} @endif @else
            <div class="more-review">
               <a href="<?php echo url('product_review_all/'.$pro_details_by_id->pro_id); ?>">More Review</a>
            </div>
		@endif 
		@else @if (Lang::has(Session::get('lang_file').'.NO_RATING_FOR_THIS_PRODUCT')!= '') {{  trans(Session::get('lang_file').'.NO_RATING_FOR_THIS_PRODUCT') }} @else {{ trans($OUR_LANGUAGE.'.NO_RATING_FOR_THIS_PRODUCT') }} @endif @endif
         </div>
	...
	@php	$i=0; @endphp 
         <div class="Similer-product">
             ...
             @php   $alt_text   = substr($product_det->$pro_title,0,25); @endphp
				...
				@php	  $i++; @endphp
                     ...
                     <div class="item">
                        <div class="imgTitle">
							...
							@if($mcat != '' && $smcat != '' && $sbcat != '' && $ssbcat != '')  
								<a href="{{ url('productview/'.$mcat.'/'.$smcat.'/'.$sbcat.'/'.$ssbcat.'/'.$res) }}"><img alt="{{ $alt_text }}" src="{{ $prod_path }}" /></a>
					  @endif 
							@if($mcat != '' && $smcat != '' && $sbcat != '' && $ssbcat == '')  
								<a href="{{ url('productview/'.$mcat.'/'.$smcat.'/'.$sbcat.'/'.$res) }}"><img alt="{{ $alt_text }}" src="{{ $prod_path }}"></a>
					  @endif
							@if($mcat != '' && $smcat != '' && $sbcat == '' && $ssbcat == '')  
								<a href="{{ url('productview/'.$mcat.'/'.$smcat.'/'.$res) }}"><img alt="{{ $alt_text }}" src="{{ $prod_path }}"></a>
					 @endif
                        </div>
                     ...
                     {{ substr ($product_det->$pro_title,0,20) }}
                        ...
                        <h5 class="">{{ Helper::cur_sym() }} {{ $product_det->pro_disprice }}</h5>
                     </div>
			...
			@endforeach <!-- //foreach -->   
			   ...
			   @if($i==0) @if (Lang::has(Session::get('lang_file').'.NO_PRODUCTS_AVAILABLE')!= '') {{ trans(Session::get('lang_file').'.NO_PRODUCTS_AVAILABLE') }} @else {{ trans($OUR_LANGUAGE.'.NO_PRODUCTS_AVAILABLE') }} @endif
			   @else 
...
<div class="your-review" id="review-comment">
   <h5 class="pro-description-title">@if (Lang::has(Session::get('lang_file').'.WRITE_A_POST_COMMENTS')!= '') {{ trans(Session::get('lang_file').'.WRITE_A_POST_COMMENTS') }} @else {{ trans($OUR_LANGUAGE.'.WRITE_A_POST_COMMENTS') }} @endif</h5>
{!! Form::open(array('url'=>'productcomments','class'=>'form-horizontal loginFrm')) !!}
   ...
   <label class="control-label review_ratings_label" for="review_ratings" style="text-align: left;"><h4>@if (Lang::has(Session::get('lang_file').'.RATE_THIS_PRODUCT')!= '') {{ trans(Session::get('lang_file').'.RATE_THIS_PRODUCT') }} @else {{ trans($OUR_LANGUAGE.'.RATE_THIS_PRODUCT') }} @endif</h4></label>
...
<div>
   <label class="control-label review_ratings_label" for="review_ratings" style="text-align: left;"><h4>@if (Lang::has(Session::get('lang_file').'.REVIEW_THIS_PRODUCT')!= '') {{ trans(Session::get('lang_file').'.REVIEW_THIS_PRODUCT') }} @else {{ trans($OUR_LANGUAGE.'.REVIEW_THIS_PRODUCT') }} @endif</h4></label>
