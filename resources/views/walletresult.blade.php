...
$favimgpath="/public/assets/favicon/".$favicon_images->imgs_name;
...
{*/
$favimgpath="/public/assets/favicon/ecartfavicon.ico";
//}
?>
...
<!-- Sidebar ================================================== -->
	<div class="span3" id="sidebar">
		<div class="well well-small btn-warning"><strong><?php if (Lang::has(Session::get('lang_file').'.CATEGORIES')!= '') { echo  trans(Session::get('lang_file').'.CATEGORIES');}  else { echo trans($OUR_LANGUAGE.'.CATEGORIES');} ?> </strong></div>
			...
			<div class="caption product_show">
				<h4 class="top_text dolor_text">$<?php echo $fetch_most_visit_pro->pro_disprice;  ?></h4>
					 ...
					 <?php if($fetch_most_visit_pro->pro_no_of_purchase >= $fetch_most_visit_pro->pro_qty) { ?>
                      <h4 style="text-align:center;"><a  class="btn btn-danger">
							  <?php if (Lang::has(Session::get('lang_file').'.SOLD')!= '') { echo  trans(Session::get('lang_file').'.SOLD');} 
							  else { echo trans($OUR_LANGUAGE.'.SOLD');} ?></a> 
					  </h4>
                      <?php } 					  
					  else { ?>
              ...
              <thead>
                ...
                <th><?php if (Lang::has(Session::get('lang_file').'.NAME')!= '') { echo  trans(Session::get('lang_file').'.NAME');}  else { echo trans($OUR_LANGUAGE.'.NAME');} ?></th>
                  <th><?php if (Lang::has(Session::get('lang_file').'.TRANSACTIONID')!= '') { echo  trans(Session::get('lang_file').'.TRANSACTIONID');}  else { echo trans($OUR_LANGUAGE.'.TRANSACTIONID');} ?></th>
                   <th><?php if (Lang::has(Session::get('lang_file').'.STATUS')!= '') { echo  trans(Session::get('lang_file').'.STATUS');}  else { echo trans($OUR_LANGUAGE.'.STATUS');} ?></th>
				</tr>
              </thead>
              <tbody>
  ...
  <h5> <?php if (Lang::has(Session::get('lang_file').'.PRODUCT_DETAILS_FOR_CURRENT_TRANSACTION')!= '') { echo  trans(Session::get('lang_file').'.PRODUCT_DETAILS_FOR_CURRENT_TRANSACTION');}  else { echo trans($OUR_LANGUAGE.'.PRODUCT_DETAILS_FOR_CURRENT_TRANSACTION');} ?> </h5>	
          ...
          ?>
				 ...
				 <tr>
                  ...
                  <td>{{ Helper::cur_sym() }} <?php echo $orderdet->order_amt + $taxamount_au;?>(including <?php echo $orderdet->order_tax;?> % tax)</td>
      			 </tr>
				<?php }
				} ?>
                ...
                <tr>                  
                    ...
                    <td style="font-weight:bold;">{{ Helper::cur_sym() }} <?php $subtotal = ($get_subtotal+$taxamount)-$coupon; echo round($subtotal,2);?> </td>             
                </tr> 

                  <tr>                  
                        <td>&nbsp;</td>                  
                        <td style=""><?php if (Lang::has(Session::get('lang_file').'.SHIPPING_TOTAL')!= '') { echo  trans(Session::get('lang_file').'.SHIPPING_TOTAL');}  else { echo trans($OUR_LANGUAGE.'.SHIPPING_TOTAL');} ?></td>                  
                        <td style="">{{ Helper::cur_sym() }} <?php  echo $shipping_amt; ?></td>            
                    </tr>
