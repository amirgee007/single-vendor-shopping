...
{!!$merchantleftmenus!!}
		...
		<!--  <form  action="{!!action('TransactionController@adm_cod_return_orders')!!}" method="POST"> -->
		 	{{ Form::open(array('action'=>'TransactionController@adm_payu_return_orders','method'=>'POST')) }}
...
placeholder="DD/MM/YYYY" class="form-control" id="datepicker-8"  value="<?php echo $to_date; ?>" required readonly> -->
...
placeholder="DD/MM/YYYY" id="datepicker-9" class="form-control" value="<?php echo $from_date; ?>" required readonly> -->
                               ...
                               <div class="col-sm-2">
								<a href="{{ url('').'/adm_payu_return_orders' }}">
...
<!-- 	    	<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button> -->
 ...
 @if(isset($_POST['submit']))
			...
			@if($returned_order_all !="") 		
			@foreach($returned_order_all as $allorders_list) 
			    ...
			    <td> Order Returned</td>
			      @endif @else <td> Order Returned</td> @endif @endif	
...
@else
				...
				<td class="center">@if($allorders_list->order_id==$allorders_list->paypal_order_id) @if($allorders_list->return_status==1)
					<a href="javascript:void(0);"  class="btn btn-info" onclick="show_Return_approved('<?php echo $allorders_list->delStatus_id;?>');">Return Request</a> @endif @endif
				</td>@elseif($allorders_list->delivery_status == 8)  @if($allorders_list->order_id==$allorders_list->paypal_order_id) 
				@if($allorders_list->return_status==2)
                <td class="center">
				  <a href="javascript:void(0);" class="btn btn-info" onclick="show_Return_approved('<?php echo $allorders_list->delStatus_id;?>');">Return Request Processed</a>
			    </td>
			    @else
...
{ 
	...
	$("#approve_Return").load('<?php echo url(''); ?>/adm_get_approve_REturncontent_payu/'+data);
}
</script>
</body>
     <!-- END BODY -->
