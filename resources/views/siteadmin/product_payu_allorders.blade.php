...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }} | {{(Lang::has(Session::get('admin_lang_file').'.BACK_PRODUCTS_ALL_ORDERS')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_PRODUCTS_ALL_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_PRODUCTS_ALL_ORDERS') }}</title>
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a >{{ (Lang::has(Session::get('admin_lang_file').'.BACK_PRODUCTS_ALL_ORDERS')!= '')? trans(Session::get('admin_lang_file').'.BACK_PRODUCTS_ALL_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_PRODUCTS_ALL_ORDERS') }}</a></li>
                            </ul>
                    </div>
                </div>
				 <center><div class="cal-search-filter">
				 	{{ Form::open(array('action'=>'TransactionController@product_payu_all_orders','method'=>'POST')) }}
...
{{ Form::text('to_date',$to_date,array('id'=>'datepicker-9','class'=>'form-control','placeholder'=>'DD/MM/YYYY','required'=>'required','readonly'=>'readonly')) }}
							<!--  <input type="text" name="to_date" placeholder="DD/MM/YYYY" id="datepicker-9" class="form-control" value="{{$to_date}}" required readonly> -->
							 ...
							 <div class="col-sm-2">
								<a href="{{ url('').'/product_payu_all_orders' }}"><button type="button" name="reset" class="btn btn-block btn-info">@if (Lang::has(Session::get('admin_lang_file').'.BACK_RESET')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_RESET') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_RESET') }} @endif</button></a>
							 </div>
							</div>
...
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>{{ (Lang::has(Session::get('admin_lang_file').'.BACK_PRODUCTS_ALL_ORDERS')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_PRODUCTS_ALL_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_PRODUCTS_ALL_ORDERS') }}</h5>
			...
			@php	$total_tax = ($allorders_list->order_amt * $allorders_list->order_tax)/100 ; @endphp
					...
					<tr class="gradeA odd">
						...
						<td class="     "> {{ $allorders_list->pro_title }}</td>
                                            ...
                                            <td class="center"><span class="colr3">{{ $orderstatus }}</span></td>
                                            <td class="center">{{ $allorders_list->order_date }}</td>
                                            <td class="center"><span class="colr2">{{ $ordertype }}</span></td>
