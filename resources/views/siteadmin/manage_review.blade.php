...
<head>
	...
	<title>{{ $SITENAME }} | @if (Lang::has(Session::get('admin_lang_file').'.BACK_MANAGE_PRODUCT_REVIEWS')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_MANAGE_PRODUCT_REVIEWS') }}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_MANAGE_PRODUCT_REVIEWS')}} @endif </title>
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a >  @if (Lang::has(Session::get('admin_lang_file').'.BACK_MANAGE_PRODUCT_REVIEWS')!= '') {{ trans(Session::get('admin_lang_file').'.BACK_MANAGE_PRODUCT_REVIEWS')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_MANAGE_PRODUCT_REVIEWS')}} @endif                  </a></li>
                            </ul>
                    </div>
                </div>
...
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>  @if (Lang::has(Session::get('admin_lang_file').'.BACK_MANAGE_PRODUCT_REVIEWS')!= '') {{ trans(Session::get('admin_lang_file').'.BACK_MANAGE_PRODUCT_REVIEWS')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_MANAGE_PRODUCT_REVIEWS')}} @endif                               </h5>
               ...
               @php $i = 1 ; @endphp
				...
				@foreach($get_review as $row) 
                                    ...
                                    <tr class="gradeA odd">
                                            ...
                                            <td class="center  ">{{  $row->cus_email }}</td>
                                             
                                             <td class="center  "><a href="#">
                                           <a href="{{ url('edit_review/'.$row->comment_id) }}" data-tooltip="@if (Lang::has(Session::get('admin_lang_file').'.BACK_EDIT')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_EDIT')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_EDIT')}} @endif"> <i class="icon icon-edit" style="margin-left:15px;"></i></a>
                                           @if($row->status == 0)
                                            <a href="{{ url('block_review/'.$row->comment_id.'/1') }}" data-tooltip="@if (Lang::has(Session::get('admin_lang_file').'.BACK_BLOCK')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_BLOCK')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_BLOCK')}} @endif"> <i style='margin-left:10px;' class='icon icon-ok icon-me'></i> </a>
                                             @elseif($row->status == 1) 
                                             <a href="{{ url('block_review/'.$row->comment_id.'/0') }}" data-tooltip="@if (Lang::has(Session::get('admin_lang_file').'.BACK_UNBLOCK')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_UNBLOCK')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_UNBLOCK')}} @endif"> <i style='margin-left:10px;' class='icon icon-ban-circle icon-me'></i> </a>
                                             @endif
                                             <a href="{{ url('delete_review')."/".$row->comment_id }}" data-tooltip="@if (Lang::has(Session::get('admin_lang_file').'.BACK_DELETE')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_DELETE')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_DELETE')}} @endif"><i class="icon icon-trash icon-1x" style="margin-left:14px;"></i></a> 
                                             <?php ?>
                                            </td>
  ...
  $(function(){
      ...
      $('#Block_value').click(function(){
        ...
        $.ajax({
          ...
          type:'GET',
          url :"<?php echo url("block_pro_review_multiple"); ?>",
          data:{val:val},
   ...
   $(function(){
      ...
      $('#UNBlock_value').click(function(){
        ...
        $.ajax({
          ...
          type:'GET',
          url :"<?php echo url("unblock_pro_review_multiple"); ?>",
          data:{val:val},
  ...
  $(function(){
      ...
      $('#Delete_value').click(function(){
        ...
        $.ajax({
          ...
          type:'GET',
          url :"<?php echo url("delete_pro_review_multiple"); ?>",
          data:{val:val},
