...
{!!$merchantleftmenus!!}
		...
		<!--  <form  action="{!!action('TransactionController@adm_cod_cancel_orders')!!}" method="POST"> -->
		 	{{ Form::open(array('action'=>'TransactionController@adm_cod_cancel_orders','method'=>'POST')) }}
...
placeholder="DD/MM/YYYY" id="datepicker-9" class="form-control" value="<?php echo $to_date; ?>" required readonly> -->
                               ...
                               <div class="col-sm-2">
								<a href="{{ url('').'/adm_cod_cancel_orders' }}">
...
<div class="col-lg-12">
 ...
 @if(isset($_POST['submit']))
			...
			@if($calceled_order !="") 
			@foreach($calceled_order as $allorders_list) 
			...
			@php		
					...
					<tr class="gradeA odd">
						...
						<?php /* if($allorders_list->delivery_status == 5) {  ?> <td class="center">
					   <?php if($allorders_list->cod_id==$calceled_order_all[0]->cod_order_id){ if($calceled_order_all[0]->cancel_status==1){?><a href="javascript:void(0);"  class="btn btn-info" onclick="show_cancel_approved('<?php echo $calceled_order_all[0]->delStatus_id;?>');">Cancel Request</a> <?php } else{?><a href="javascript:void(0);"  class="btn btn-info" onclick="show_cancel_approved('<?php echo $calceled_order_all[0]->delStatus_id;?>');">Cancel Request Processed</a> <?php } }?>
							</td> <?php  } else {  ?> 
					   <td class="center">
					  Order Cancelled
					  </td> <?php } */ ?>
						 ...
						 @if($allorders_list->delivery_status == 5) 
						  ...
						  @elseif($allorders_list->delivery_status == 6) 
						   @if($allorders_list->cod_id==$allorders_list->cod_order_id) @if($allorders_list->cancel_status==2)  
								<td class="center">
...
function show_cancel_approved(data)
{
	...
	$("#approve_cancel").load('<?php echo url(''); ?>/adm_get_approve_content/'+data);
}
</script>
</body>
     <!-- END BODY -->
