...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }} | @if (Lang::has(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_HOLD_ORDERS') }} @endif</title>
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a>@if (Lang::has(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS')!= '') {{ trans(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_HOLD_ORDERS') }} @endif</a></li>
                            </ul>
                    </div>
                </div>
...
placeholder="DD/MM/YYYY"  class="form-control" id="datepicker-8" value="{{$from_date}}" required readonly> -->
 ...
 <span class="icon-calendar cale-icon"></span>
  {{ Form::text('to_date',$to_date,array('id'=>'datepicker-9','class'=>'form-control','placeholder'=>'DD/MM/YYYY','required'=>'required','readonly'=>'readonly')) }}
							 <!-- <input type="text" 
placeholder="DD/MM/YYYY"  name="to_date"  id="datepicker-9" class="form-control" value="{{$to_date}}" required readonly> -->
                             ...
                             <div class="col-sm-2">
								<a href="{{ url('').'/deals_hold_orders' }}"><button type="button" name="reset" class="btn btn-block btn-info">@if (Lang::has(Session::get('admin_lang_file').'.BACK_RESET')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_RESET') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_RESET') }} @endif</button></a>
							 </div>
							</div>
...
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>@if (Lang::has(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_HOLD_ORDERS') }} @endif</h5>
        ...
        <div id="div-1" class="accordion-body collapse in body">
		   ...
		   <table id="dataTables-example" class="table table-striped table-bordered table-hover dataTable no-footer" aria-describedby="dataTables-example_info">
				...
				<th class="sorting_asc" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 69px;" aria-label="S.No: activate to sort column ascending" aria-sort="ascending">@if (Lang::has(Session::get('admin_lang_file').'.BACK_SNO')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_SNO') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_SNO') }} @endif</th>
					...
					<th class="sorting" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 88px;" aria-label="Edit: activate to sort column ascending">@if (Lang::has(Session::get('admin_lang_file').'.BACK_TRANSACTION_TYPE')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_TRANSACTION_TYPE') }}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_TRANSACTION_TYPE') }} @endif</th>
			...
			@php	$ordertype = ((Lang::has(Session::get('admin_lang_file').'.BACK_PAYPAL')!= ''))? trans(Session::get('admin_lang_file').'.BACK_PAYPAL') : trans($ADMIN_OUR_LANGUAGE.'.BACK_PAYPAL');	
			...
			@php	$total_tax = ($row->order_amt * $row->order_tax)/100 ; @endphp
					...
					<tr class="gradeA odd">
                                           ...
                                           <td class="center     ">
											...
											<select name="<?php  echo 'status'.$row->order_id;?>" class="btn btn-default" onchange="update_order_paypal(this.value,'<?php echo $row->transaction_id;?>','<?php echo $row->deal_id;?>')">
			...
			@php $total_tax = ($allorders_list->order_amt * $allorders_list->order_tax)/100 ; @endphp
                  ...
                  <select name="<?php  echo 'status'.$allorders_list->order_id;?>" class="btn btn-default" onchange="update_order_paypal(this.value,'<?php echo $allorders_list->transaction_id;?>','<?php echo $allorders_list->deal_id;?>')">
	...
	function update_order_paypal(id,orderid,proid)
	{
			 ...
			 var pathnametemp =$('#return_url').val();
			  $.ajax( {
			      type: 'get',
				  data: passdata,
				  url: 'update_deal_order_paypal_admin',
				   beforeSend: function() {
				        // setting a timeout
