...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }} | {{ (Lang::has(Session::get('admin_lang_file').'.BACK_MANAGE_SPECIFICATION')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_MANAGE_SPECIFICATION') : trans($ADMIN_OUR_LANGUAGE.'.BACK_MANAGE_SPECIFICATION') }}</title>
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a >{{ (Lang::has(Session::get('admin_lang_file').'.BACK_MANAGE_SPECIFICATION')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_MANAGE_SPECIFICATION')  : trans($ADMIN_OUR_LANGUAGE.'.BACK_MANAGE_SPECIFICATION') }}</a></li>
                            </ul>
                    </div>
                </div>
            <div class="row">
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>{{ (Lang::has(Session::get('admin_lang_file').'.BACK_MANAGE_SPECIFICATION')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_MANAGE_SPECIFICATION') : trans($ADMIN_OUR_LANGUAGE.'.BACK_MANAGE_SPECIFICATION') }}</h5>
   ...
   <div class="table-responsive panel_marg_clr ppd">
    	<table  class="table table-bordered" id="dataTables-example">
		 ...
		 <tr>
                  ...
                  <th>{{ (Lang::has(Session::get('admin_lang_file').'.BACK_SPECIFICATION_NAME')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_SPECIFICATION_NAME') : trans($ADMIN_OUR_LANGUAGE.'.BACK_SPECIFICATION_NAME') }}</th>
 ...
 <?php $i=1; 
//print_r($specificationresult);exit();
?> 
        @if(count($specificationresult)>0)
                ...
                <tr>
                  ...
                  <td class="text-center">{!!$info->sp_name!!}</td>
				     <td class="text-center">@if($info->spg_name) {!!$info->spg_name!!} @else {{ "-" }} @endif</td>
					  <td class="text-center">{!!$info->sp_order!!}</td>
				   <td class="text-center">
<a href="{{ url('edit_specification/'.$info->sp_id) }}" data-tooltip="Edit"><i class="icon icon-edit icon-2x"></i></a></td>
...
->where('nm_prospec.spc_sp_id','=',$info->sp_id)
->count(); @endphp
				   <!--td class="text-center">
                  </td-->
...
@if($spec_added_product<1)
                   <a href="{{ url('delete_specification/'.$info->sp_id) }}" data-tooltip="Delete"><i class="icon icon-trash icon-2x"></i></a>
