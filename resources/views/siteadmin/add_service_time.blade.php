  ...
  <div class="col-lg-12">
    <div class="box dark">
        ...
        <div id="div-1" class="accordion-body collapse in body col-lg-11 panel_marg" style="padding-bottom:10px;">
             {!! Form::open(array('url'=>'add_service_time_submit','class'=>'form-horizontal')) !!}
			 
			 <div class="panel panel-default">

                <div class="panel-heading"> <?php if (Lang::has(Session::get('admin_lang_file').'.BACK_ADD_SERVICE_TIME')!= '') { echo  trans(Session::get('admin_lang_file').'.BACK_ADD_SERVICE_TIME');}  else { echo trans($ADMIN_OUR_LANGUAGE.'.BACK_ADD_SERVICE_TIME');} ?> </div>
			 
                    <div class="panel-body">
                        <div class="form-group">
                                <label class="control-label col-lg-3" for="text1"><?php if (Lang::has(Session::get('admin_lang_file').'.BACK_SERVICE_TIME')!= '') { echo  trans(Session::get('admin_lang_file').'.BACK_SERVICE_TIME');}  else { echo trans($ADMIN_OUR_LANGUAGE.'.BACK_SERVICE_TIME');} ?><span class="text-sub">*</span></label>
                                <div class="col-lg-2">
                                    <div id="datetimepicker1" class="date input-group">
                                        <input type="hidden" name="days_name[]" value="sunday">
                                        <input type="text" id="service_time" name="service_time" class="form-control" placeholder="<?php if (Lang::has(Session::get('admin_lang_file').'.BACK_ENTER_SERVICE_TIME')!= '') { echo  trans(Session::get('admin_lang_file').'.BACK_ENTER_SERVICE_TIME');}  else { echo trans($ADMIN_OUR_LANGUAGE.'.BACK_ENTER_SERVICE_TIME');} ?>" required></input>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-4">
						        <label class="sample checkbox-inline" id="chk_radio">
                                    <input type="radio" name="Service_time_status" value="1" checked> <span><?php if (Lang::has(Session::get('admin_lang_file').'.BACK_ACTIVE')!= '') { echo  trans(Session::get('admin_lang_file').'.BACK_ACTIVE');}  else { echo trans($ADMIN_OUR_LANGUAGE.'.BACK_ACTIVE');} ?></span>
                                </label>
						        <label class="sample checkbox-inline" id="chk_radio">
						            <input type="radio"  name="Service_time_status" value="0"> <span><?php if (Lang::has(Session::get('admin_lang_file').'.BACK_DEACTIVE')!= '') { echo  trans(Session::get('admin_lang_file').'.BACK_DEACTIVE');}  else { echo trans($ADMIN_OUR_LANGUAGE.'.BACK_DEACTIVE');} ?></span>
						        </label>
						    </div>
                        </div> 
                    </div>
...
$(document).keydown(function(event){
    ...
    return false;
      ...
      return false;  //ffPrevent from ctrl+shift+i
   }
});
</script>
<!--Time picker-->
