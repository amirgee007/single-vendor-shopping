...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }} @if (Lang::has(Session::get('admin_lang_file').'.MER_CANCEL_ORDERS')!= '') {{  trans(Session::get('admin_lang_file').'.MER_CANCEL_ORDERS') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.MER_CANCEL_ORDERS') }} @endif</title>
...
{!!$merchantleftmenus!!}
        ...
        <div id="content">
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                            	<li class=""><a href="<?php echo url('siteadmin_dashboard'); ?>">@if (Lang::has(Session::get('admin_lang_file').'.BACK_HOME')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_HOME') }}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_HOME') }} @endif</a></li>
                                <li class="active"><a href="#">@if (Lang::has(Session::get('admin_lang_file').'.BACK_CANCELLED_ORDER')!= '') {{ trans(Session::get('admin_lang_file').'.BACK_CANCELLED_ORDER') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_CANCELLED_ORDER') }} @endif</a></li>
                            </ul>
                    </div>
                </div>
				 <center><div class="cal-search-filter">
		 <form  action="{!!action('TransactionController@adm_deal_payu_cancel_orders')!!}" method="POST">
							<input type="hidden" name="_token" placeholder="DD/MM/YYYY"  value="<?php echo csrf_token(); ?>">
							 <div class="row">
							 <br>
...
placeholder="DD/MM/YYYY"  id="datepicker-9" class="form-control" value="<?php echo $to_date; ?>" required readonly> -->
                               ...
                               <div class="col-sm-2">
								<a href="{{ url('').'/adm_deal_payu_cancel_orders' }}">
...
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>@if (Lang::has(Session::get('admin_lang_file').'.BACK_CANCELLED_ORDER')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_CANCELLED_ORDER') }}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_CANCELLED_ORDER') }} @endif</h5>
 ...
 @if(isset($_POST['submit']))
			...
			@if($calceled_order_alldeal !="") 
			@foreach($calceled_order_alldeal as $allorders_list) 
				...
				@php	$total_tax = ($allorders_list->order_amt * $allorders_list->order_tax)/100 ; @endphp
					...
					@if($allorders_list->delivery_status == 5) 
							<td class="center">@if($allorders_list->order_id==$allorders_list->paypal_order_id)
								@if($allorders_list->cancel_status==1)<a href="javascript:void(0);"  class="btn btn-info" onclick="show_cancel_approved('{{ $allorders_list->delStatus_id }}');">Cancel Request</a> @endif @endif
							</td> @elseif($allorders_list->delivery_status == 6)  @if($allorders_list->order_id==$allorders_list->paypal_order_id) @if($allorders_list->cancel_status==2)  
							<td class="center">
								<a href="javascript:void(0);"  class="btn btn-info" onclick="show_cancel_approved('{{ $allorders_list->delStatus_id }}');">Cancel Request Processed</a>
							</td>
						@else  <td>Order Cancelled</td>	
							 @endif  @else  <td>Order Cancelled</td>
					 @endif @endif
	...
	<!-- Modal content-->
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">Approve cancel Status</h4>
			</div>
...
function show_cancel_approved(data)
{
	...
	$("#approve_cancel").load('<?php echo url(''); ?>/adm_deal_get_approve_content_payu/'+data);
}
</script>
</body>
     <!-- END BODY -->
