...
<html lang="en" class="ie8">
   ...
   <html lang="en" class="ie9">
      ...
      <html lang="en">
         ...
         <head>
            <meta charset="UTF-8" />
            <title>{{ $SITENAME }}  |{{ (Lang::has(Session::get('admin_lang_file').'.BACK_PRODUCTS_FAILED_ORDERS')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_PRODUCTS_FAILED_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_PRODUCTS_FAILED_ORDERS') }}</title>
               ...
               <div class="inner">
                  <div class="row">
                     <div class="col-lg-12">
                        <ul class="breadcrumb">
                           ...
                           <li class="active"><a >{{ (Lang::has(Session::get('admin_lang_file').'.BACK_PRODUCTS_FAILED_ORDERS')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_PRODUCTS_FAILED_ORDERS'): trans($ADMIN_OUR_LANGUAGE.'.BACK_PRODUCTS_FAILED_ORDERS') }}
                              </a>
                           </li>
                        </ul>
                     </div>
                  ...
                  <center>
                     <div class="cal-search-filter">
                        {{ Form::open(array('action'=>'TransactionController@product_payu_failed_orders','method'=>'POST')) }}
                        ...
                        <div class="row">
                           ...
                           <div class="form-group">
                              ...
                              <div class="col-sm-2">
                                 <a href="{{ url('').'/product_payu_failed_orders' }}"><button type="button" name="reset" class="btn btn-block btn-info">@if (Lang::has(Session::get('admin_lang_file').'.BACK_RESET')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_RESET') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_RESET') }} @endif</button></a>
                              </div>
                           </div>
                           {{ Form::close()}}
                        </div>
                  ...
                  <h5>{{ (Lang::has(Session::get('admin_lang_file').'.BACK_PRODUCTS_FAILED_ORDERS')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_PRODUCTS_FAILED_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_PRODUCTS_FAILED_ORDERS') }}</h5>
                  </header>
                  ...
                  @else
                  <select disabled name="<?php  echo 'status'.$allorders_list->order_id;?>" class="btn btn-default" onchange="update_order_paypal(this.value,'<?php echo $allorders_list->transaction_id;?>','<?php echo $allorders_list->pro_id;?>')">
                  <option value="1" <?php if($allorders_list->delivery_status==2 || $allorders_list->delivery_status==3 || $allorders_list->delivery_status==4) {?>style="display:none" <?php } ?><?php if($allorders_list->delivery_status==1){?> selected <?php } ?>>@if (Lang::has(Session::get('admin_lang_file').'.BACK_ORDER_PLACED')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_ORDER_PLACED') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_ORDER_PLACED') }} @endif</option>
                  ...
                  <option value="3" <?php if($allorders_list->delivery_status==4) {?>style="display:none" <?php } ?> <?php if($allorders_list->delivery_status==3){?> selected <?php } ?>>@if (Lang::has(Session::get('admin_lang_file').'.BACK_DISPATCHED')!= '') {{ trans(Session::get('admin_lang_file').'.BACK_DISPATCHED') }}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_DISPATCHED') }} @endif</option>
                  <option value="4" <?php if($allorders_list->delivery_status==4){?> selected <?php } ?>>@if (Lang::has(Session::get('admin_lang_file').'.BACK_DELIVERED')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_DELIVERED') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_DELIVERED') }} @endif</option>
                  ...
                  @if($allorders_list->order_paytype==1)
                  @php      
                  $ordertype = ((Lang::has(Session::get('admin_lang_file').'.BACK_PAYUMONEY')!= ''))? trans(Session::get('admin_lang_file').'.BACK_PAYUMONEY') : trans($ADMIN_OUR_LANGUAGE.'.BACK_PAYUMONEY'); @endphp
                  @elseif($allorders_list->order_paytype==2)
                  @php
               ...
               function update_order_paypal(id,orderid,proid)
               {
                     ...
                     var pathnametemp =$('#return_url').val();
                      $.ajax( {
                          type: 'get',
                          data: passdata,
                          url: 'update_payu_order_status_admin',
                           beforeSend: function() {
                                // setting a timeout
