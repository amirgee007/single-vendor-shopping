...
{!!$adminleftmenus!!}
        ...
        <div id="content">
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a>  {{ (Lang::has(Session::get('admin_lang_file').'.BACK_COD_FAILED_ORDERS')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_COD_FAILED_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_COD_FAILED_ORDERS') }}</a></li>
                            </ul>
                    </div>
                </div>
		 ...
		 <!-- <form  action="{!!action('TransactionController@cod_failed_orders')!!}" method="POST"> -->
		 	{{ Form::open(array('action'=>'TransactionController@cod_failed_orders','method'=>'POST')) }}
 ...
 {{ Form::text('to_date',$to_date,array('id'=>'datepicker-9','class'=>'form-control','placeholder'=>'DD/MM/YYYY','required'=>'required','readonly'=>'readonly')) }}
							<!--  <input type="text" name="to_date" placeholder="DD/MM/YYYY"  id="datepicker-9" class="form-control" value="{{$to_date}}" required readonly> -->
                               ...
                               <div class="col-sm-2">
							    <a href="{{ url('').'/cod_failed_orders' }}"><button type="button" name="reset" class="btn btn-block btn-info">@if (Lang::has(Session::get('admin_lang_file').'.BACK_RESET')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_RESET') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_RESET') }} @endif</button></a>
							   </div>
							</div>
...
<div class="col-lg-12">
	...
	@if(isset($_POST['submit']))
				...
				@php	
					$orderstatus = ((Lang::has(Session::get('admin_lang_file').'.BACK_HOLD')!= ''))? trans(Session::get('admin_lang_file').'.BACK_HOLD') : trans($ADMIN_OUR_LANGUAGE.'.BACK_HOLD'); @endphp
				...
				@php	
					...
					<tr class="gradeA odd">
                                            ...
                                            <td class="center     ">{{ $allorders_list->pro_title }}</td>
                                            <td class="center     ">{{ $allorders_list->cod_amt }}</td>
                                            <td class="center     ">{{ $allorders_list->cod_tax }}</td>
				...
				@php	$ordertype = ((Lang::has(Session::get('admin_lang_file').'.BACK_COD')!= ''))? trans(Session::get('admin_lang_file').'.BACK_COD') : trans($ADMIN_OUR_LANGUAGE.'.BACK_COD');	 @endphp
					...
					<tr class="gradeA odd">
                                          ...
                                          @if($allorders_list->delivery_status==5)
                                           	 ...
                                           	 <span>Replace Request Pending</span>
                                           	 @else
                  <select disabled name="<?php  echo 'status'.$allorders_list->cod_id;?>" class="btn btn-default" onchange="update_order_cod(this.value,'<?php echo $allorders_list->cod_transaction_id;?>','<?php echo $allorders_list->pro_id;?>')">
                                              <option value="1" <?php if($allorders_list->delivery_status==2 || $allorders_list->delivery_status==3 || $allorders_list->delivery_status==4) {?>style="display:none" <?php } ?><?php if($allorders_list->delivery_status==1){?> selected <?php } ?>>@if (Lang::has(Session::get('admin_lang_file').'.BACK_ORDER_PLACED')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_ORDER_PLACED') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_ORDER_PLACED') }} @endif</option>
