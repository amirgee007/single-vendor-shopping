...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }}| @if (Lang::has(Session::get('admin_lang_file').'.BACK_SUCCESS_ORDERS')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_SUCCESS_ORDERS') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_SUCCESS_ORDERS') }} @endif</title>
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a>@if (Lang::has(Session::get('admin_lang_file').'.BACK_SUCCESS_ORDERS')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_SUCCESS_ORDERS') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_SUCCESS_ORDERS') }} @endif</a></li>
                            </ul>
                    </div>
                </div>
			<center><div class="cal-search-filter">
		 <form  action="{!!action('TransactionController@deals_payu_success_orders')!!}" method="POST">
 ...
 <span class="icon-calendar cale-icon"></span>
  {{ Form::text('to_date',$to_date,array('id'=>'datepicker-9','class'=>'form-control','placeholder'=>'DD/MM/YYYY','required'=>'required','readonly'=>'readonly')) }}
							<!--  <input type="text" name="to_date" placeholder="DD/MM/YYYY" id="datepicker-9" class="form-control" value="{{$to_date}}" required readonly> -->
                             ...
                             <div class="col-sm-2">
								<a href="{{ url('').'/deals_payu_success_orders' }}"><button type="button" name="reset" class="btn btn-block btn-info">@if (Lang::has(Session::get('admin_lang_file').'.BACK_RESET')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_RESET') }}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_RESET') }} @endif</button></a>
							 </div>
							</div>
...
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>@if (Lang::has(Session::get('admin_lang_file').'.BACK_SUCCESS_ORDERS')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_SUCCESS_ORDERS') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_SUCCESS_ORDERS') }} @endif</h5>
