...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }} {{ (Lang::has(Session::get('admin_lang_file').'.BACK_COD_RETURN_ORDERS')!= '')?  trans(Session::get('admin_lang_file').'.BACK_COD_RETURN_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_COD_RETURN_ORDERS') }}</title>
...
{!!$merchantleftmenus!!}
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a href="#">{{ (Lang::has(Session::get('admin_lang_file').'.BACK_COD_RETURN_ORDERS')!= '')?  trans(Session::get('admin_lang_file').'.BACK_COD_RETURN_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_COD_RETURN_ORDERS') }}</a></li>
                            </ul>
                    </div>
                </div>
				  <center><div class="cal-search-filter">
				  	{{ Form::open(array('action'=>'TransactionController@adm_dealcod_return_orders','method'=>'POST')) }}
		 <!-- <form  action="{!!action('TransactionController@adm_dealcod_return_orders')!!}" method="POST"> -->
 ...
 {{ Form::text('to_date',$to_date,array('id'=>'datepicker-9','class'=>'form-control','placeholder'=>'DD/MM/YYYY','required'=>'required','readonly'=>'readonly')) }}
							<!--  <input type="text" name="to_date" placeholder="DD/MM/YYYY"  id="datepicker-9" class="form-control" value="<?php echo $to_date; ?>" required readonly> -->
                               ...
                               <div class="col-sm-2">
								<a href="<?php echo url('').'/adm_dealcod_return_orders';?>">
...
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>{{(Lang::has(Session::get('admin_lang_file').'.BACK_COD_RETURN_ORDERS')!= '')?  trans(Session::get('admin_lang_file').'.BACK_COD_RETURN_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_COD_RETURN_ORDERS') }}</h5>
        ...
        <div class="alert alert-danger alert-dismissable"  > {{ $er_msg }}
        	{{ Form::button('x',['class'=>'close closeAlert','aria-hidden'=>'true','data-dismiss'=>'alert'])}}	
 ...
 @if(isset($_POST['submit']))
			...
			@if($returned_order != "") {
			@foreach($returned_order as $allorders_list) 
			...
			@php $total_tax = ($allorders_list->cod_amt * $allorders_list->cod_tax)/100;@endphp
					...
					<tr class="gradeA odd">
					   ...
					   @if($allorders_list->delivery_status == 7) 
						   <td class="center">@if($allorders_list->cod_id==$allorders_list->cod_order_id) @if($allorders_list->return_status==1)
					...
					@elseif($allorders_list->delivery_status == 8)  @if($allorders_list->cod_id==$allorders_list->cod_order_id) @if($allorders_list->return_status==2)
							<td><a href="javascript:void(0);"  class="btn btn-info" onclick="show_Return_approved('<?php echo $allorders_list->delStatus_id;?>');">Return Request Processed</a></td>
							@endif  @else <td>Order Returned</td> 
					 @endif @endif
...
function show_Return_approved(data)
{
	...
	$("#approve_Return").load('<?php echo url(''); ?>/adm_deal_get_approve_REturncontent/'+data);
}
</script>
</body>
     <!-- END BODY -->
