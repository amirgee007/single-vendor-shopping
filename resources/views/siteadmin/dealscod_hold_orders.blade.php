...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }} |{{ (Lang::has(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS')!= '') ? trans(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_HOLD_ORDERS') }} </title>
...
{!!$adminleftmenus!!}
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a href="#"> {{ (Lang::has(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS')!= '') ? trans(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_HOLD_ORDERS') }}</a></li>
                            </ul>
                    </div>
                </div>
 ...
 {{ Form::text('to_date',$to_date,array('id'=>'datepicker-9','class'=>'form-control','placeholder'=>'DD/MM/YYYY','required'=>'required','readonly'=>'readonly')) }}
							<!--  <input type="text" name="to_date" placeholder="DD/MM/YYYY" id="datepicker-9" class="form-control"  value="{{$to_date}}" required readonly> -->
                             ...
                             <div class="col-sm-2">
								<a href="<?php echo url('').'/dealscod_hold_orders';?>"><button type="button" name="reset" class="btn btn-block btn-info">@if (Lang::has(Session::get('admin_lang_file').'.BACK_RESET')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_RESET') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_RESET') }} @endif</button></a>
							 </div>
							</div>
...
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>{{ (Lang::has(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS')!= '')  ? trans(Session::get('admin_lang_file').'.BACK_HOLD_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_HOLD_ORDERS') }}</h5>
...
$ordertype = ((Lang::has(Session::get('admin_lang_file').'.BACK_COD')!= ''))? trans(Session::get('admin_lang_file').'.BACK_COD') : trans($ADMIN_OUR_LANGUAGE.'.BACK_COD'); 
                  ...
                  <select name="<?php  echo 'status'.$row->cod_id;?>" class="btn btn-default" onchange="update_order_cod(this.value,'<?php echo $row->cod_transaction_id;?>','<?php echo $row->deal_id;?>')">
...
$ordertype = ((Lang::has(Session::get('admin_lang_file').'.BACK_COD')!= ''))? trans(Session::get('admin_lang_file').'.BACK_COD') : trans($ADMIN_OUR_LANGUAGE.'.BACK_COD');
				...
				$total_tax = ($allorders_list->cod_amt * $allorders_list->cod_tax)/100 ;	 @endphp
