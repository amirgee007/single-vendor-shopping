...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }} | @if (Lang::has(Session::get('admin_lang_file').'.BACK_DRAFTED_BLOG')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_DRAFTED_BLOG')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_DRAFTED_BLOG')}} @endif  </title>
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a>@if (Lang::has(Session::get('admin_lang_file').'.BACK_DRAFTED_BLOG')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_DRAFTED_BLOG')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_DRAFTED_BLOG')}} @endif</a></li>
                            </ul>
                    </div>
                </div>
            <div class="row">
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>@if (Lang::has(Session::get('admin_lang_file').'.BACK_DRAFTED_BLOG')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_DRAFTED_BLOG')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_DRAFTED_BLOG')}} @endif</h5>
        ...
        <div id="div-1" class="accordion-body collapse in body">
           ...
           <table aria-describedby="dataTables-example_info" class="table table-striped table-bordered table-hover dataTable no-footer" id="dataTables-example">
                                    <thead>
                                        <tr role="row">
										<th aria-label="Rendering engine: activate to sort column ascending" style="width: 100px;" colspan="1" rowspan="1" aria-controls="dataTables-example" tabindex="0" class="sorting_asc" aria-sort="ascending">@if (Lang::has(Session::get('admin_lang_file').'.BACK_SNO')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_SNO')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_SNO')}} @endif</th>
										<th aria-label="Browser: activate to sort column ascending" style="width: 100px;" colspan="1" rowspan="1" aria-controls="dataTables-example" tabindex="0" class="sorting">@if (Lang::has(Session::get('admin_lang_file').'.BACK_BLOG_TITLE')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_BLOG_TITLE')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_BLOG_TITLE')}} @endif</th>
										<th aria-label="Platform(s): activate to sort column ascending" style="width: 100px;" colspan="1" rowspan="1" aria-controls="dataTables-example" tabindex="0" class="sorting">@if (Lang::has(Session::get('admin_lang_file').'.BACK_CATEGORY')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_CATEGORY')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_CATEGORY')}} @endif</th>
										
																				<th aria-label="CSS grade: activate to sort column ascending" style="width: 100px;" colspan="1" rowspan="1" aria-controls="dataTables-example" tabindex="0" class="sorting">@if (Lang::has(Session::get('admin_lang_file').'.BACK_DATE')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_DATE')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_DATE')}} @endif</th>
										<th aria-label="CSS grade: activate to sort column ascending" style="width: 100px;" colspan="1" rowspan="1" aria-controls="dataTables-example" tabindex="0" class="sorting"> @if (Lang::has(Session::get('admin_lang_file').'.BACK_IMAGE')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_IMAGE')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_IMAGE')}} @endif </th>
										<th aria-label="CSS grade: activate to sort column ascending" style="width: 100px;" colspan="1" rowspan="1" aria-controls="dataTables-example" tabindex="0" class="sorting"> @if (Lang::has(Session::get('admin_lang_file').'.BACK_EDIT')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_EDIT')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_EDIT')}} @endif </th>	
                                        ...
                                        <th aria-label="CSS grade: activate to sort column ascending" style="width: 100px;" colspan="1" rowspan="1" aria-controls="dataTables-example" tabindex="0" class="sorting">@if (Lang::has(Session::get('admin_lang_file').'.BACK_DELETE')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_DELETE')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_DELETE')}} @endif</th>
										<th aria-label="CSS grade: activate to sort column ascending" style="width: 100px;" colspan="1" rowspan="1" aria-controls="dataTables-example" tabindex="0" class="sorting">@if (Lang::has(Session::get('admin_lang_file').'.BACK_BLOG_DETAILS')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_BLOG_DETAILS')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_BLOG_DETAILS')}}@endif</th>
									...
									<?php   $auc_get_img = $blog_list->blog_image; ?>
									  ...
									  @php  
										  $process = "<a href='".url('block_blog/'.$blog_list->blog_id.'/0/'.$blog_list->blog_type)."' > <i style='margin-left:10px;' class='icon icon-ok icon-me'></i> </a>"; @endphp
								  @elseif($blog_list->blog_status == 0) 
										<?php   $process = "<a href='".url('block_blog/'.$blog_list->blog_id.'/1/'.$blog_list->blog_type)."' > <i style='margin-left:10px;' class='icon icon-ban-circle icon-me'></i> </a>"; ?>
									  @endif
                                    <tr class="gradeA odd">
				  ...
				  @else
					 ...
					 <td class="text-center"><a href="{!! url('delete_blog_submit').'/'.$blog_list->blog_id.'/'.$blog_list->blog_type!!}" data-tooltip="@if (Lang::has(Session::get('admin_lang_file').'.BACK_DELETE')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_DELETE')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_DELETE')}} @endif"><i class="icon icon-trash icon-2x"></i></a></td>
                                            ...
                                            <td class="center  "><a href="<?php echo url('blog_details')."/".$blog_list->blog_id; ?>">@if (Lang::has(Session::get('admin_lang_file').'.BACK_VIEW_DETAILS')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_VIEW_DETAILS')}}  @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_VIEW_DETAILS')}} @endif</a></td>
                                        </tr>
									<?php $i++;  ?>
									@endforeach
									@endif
