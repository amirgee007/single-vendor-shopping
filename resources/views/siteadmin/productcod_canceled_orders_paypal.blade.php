...
{!!$merchantleftmenus!!}
        ...
        <div id="content">
				...
				<center><div class="cal-search-filter">
					{{ Form::open(array('action'=>'TransactionController@adm_cancel_orders','method'=>'POST')) }}
			...
			@foreach($calceled_order_all as $allorders_list) 
				...
				@php	$total_tax = ($allorders_list->order_amt * $allorders_list->order_tax)/100 ; @endphp
				  ...
				  @if($allorders_list->delivery_status == 5) 
							...
							</td>  @endif @else <td>Order Cancelled</td> @endif @endif
...
function show_cancel_approved(data)
{
	...
	$("#approve_cancel").load('<?php echo url(''); ?>/adm_get_approve_content_paypal/'+data);
}
</script>
</body>
     <!-- END BODY -->
