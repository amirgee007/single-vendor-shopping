...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }} {{ (Lang::has(Session::get('admin_lang_file').'.BACK_ALL_ONLINE_TRANSACTION')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_ALL_ONLINE_TRANSACTION') : trans($ADMIN_OUR_LANGUAGE.'.BACK_ALL_ONLINE_TRANSACTION') }} |{{ (Lang::has(Session::get('admin_lang_file').'.BACK_ALL_ONLINE_TRANSACTION')!= '')  ?  trans(Session::get('admin_lang_file').'.BACK_ALL_ONLINE_TRANSACTION') : trans($ADMIN_OUR_LANGUAGE.'.BACK_ALL_ONLINE_TRANSACTION') }}</title>
      ...
      <div class="row"> 
        <div class="col-lg-12"> 
          <ul class="breadcrumb">
            ...
            <li class="active"><a href="#">{{ (Lang::has(Session::get('admin_lang_file').'.BACK_ALL_ONLINE_TRANSACTION')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_ALL_ONLINE_TRANSACTION') : trans($ADMIN_OUR_LANGUAGE.'.BACK_ALL_ONLINE_TRANSACTION') }}</a>&nbsp;({{ (Lang::has(Session::get('admin_lang_file').'.BACK_PAYPAL')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_PAYPAL') : trans($ADMIN_OUR_LANGUAGE.'.BACK_PAYPAL') }} {{ "&" }} {{ (Lang::has(Session::get('admin_lang_file').'.BACK_PAYUMONEY')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_PAYUMONEY') : trans($ADMIN_OUR_LANGUAGE.'.BACK_PAYUMONEY') }})</li>
          </ul>
        </div>
      </div>
      <div class="row"> 
        ...
        <div class="col-lg-12"> 
          <div class="box dark"> <header> 
            ...
            <h5>{{ (Lang::has(Session::get('admin_lang_file').'.BACK_ALL_ONLINE_TRANSACTION')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_ALL_ONLINE_TRANSACTION') : trans($ADMIN_OUR_LANGUAGE.'.BACK_ALL_ONLINE_TRANSACTION') }}</h5>
            </header> 
            <div class="row">
                <div class="col-lg-12">                     
   ...
   <div class="table-responsive panel_marg_clr ppd">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    @php $i=1; @endphp
                                              <tr class="odd gradeX">
  <?php //PRODUCTS
  $get_details_product_coupon= $get_payu_details_product->sum('coupon_amount')+$get_paypal_details_product->sum('coupon_amount');
  $get_details_product_amt= $get_payu_details_product->sum('order_amt')+$get_paypal_details_product->sum('order_amt');
  ?>
                                                 <td> 1</td>
                                                <td>{{$get_details_product_amt}}</td>
 
                                                 <td> {{$get_details_product_coupon}}</td>
                                                ...
                                                <tr class="odd gradeX">
                                                 <td> 2</td>
<?php  //DEALS
$get_details_deals_amt=$get_paypal_details_deals->sum('order_amt')+$get_payu_details_deals->sum('order_amt');
$get_details_deals_coupon=$get_paypal_details_deals->sum('coupon_amount')+$get_payu_details_deals->sum('coupon_amount');
?>
                                                <td>{{$get_details_deals_amt}}</td>
                                               
                                                 <td> {{$get_details_deals_coupon}}</td>
