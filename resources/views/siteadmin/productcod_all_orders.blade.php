                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a >{{ (Lang::has(Session::get('admin_lang_file').'.BACK_COD_ALL_ORDERS')!= '') ? trans(Session::get('admin_lang_file').'.BACK_COD_ALL_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_COD_ALL_ORDERS') }}</a></li>
                            </ul>
                    </div>
                </div>
				
				<center><div class="cal-search-filter">
					{{ Form::open(array('action'=>'TransactionController@cod_all_orders','method'=>'POST')) }}
...
placeholder="DD/MM/YYYY" id="datepicker-9" class="form-control"value="{{$to_date}}"  required readonly> -->
							 ...
							 <div class="col-sm-2">
								<a href="<?php echo url('').'/cod_all_orders';?>"><button type="button" name="reset" class="btn btn-block btn-info">@if (Lang::has(Session::get('admin_lang_file').'.BACK_RESET')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_RESET') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_RESET') }} @endif</button></a>
							 </div>
							</div>
...
<div class="table-responsive panel_marg_clr ppd">
		...
		@foreach($product_codrep as $allorders_list) 
			...
			@php		$orderstatus="";
				...
				@php	
					$orderstatus = ((Lang::has(Session::get('admin_lang_file').'.BACK_SUCCESS')!= ''))? trans(Session::get('admin_lang_file').'.BACK_SUCCESS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_SUCCESS'); @endphp
