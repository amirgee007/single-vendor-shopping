...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }} {{ (Lang::has(Session::get('admin_lang_file').'BACK_REPLACE_ORDERS')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_REPLACE_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_REPLACE_ORDERS') }}</title>
...
{!!$merchantleftmenus!!}
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a href="#">{{ (Lang::has(Session::get('admin_lang_file').'.BACK_REPLACE_ORDERS')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_REPLACE_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_REPLACE_ORDERS') }}</a></li>
                            </ul>
                    </div>
                </div>
				<center><div class="cal-search-filter">
		 <!-- <form  action="{!!action('TransactionController@adm_replacement_orders')!!}" method="POST"> -->
		 	{{ Form::open(array('action'=>'TransactionController@adm_payu_replacement_orders','method'=>'POST')) }}
 ...
 {{ Form::text('from_date',$from_date,array('id'=>'datepicker-8','class'=>'form-control','placeholder'=>'DD/MM/YYYY','required'=>'required','readonly'=>'readonly')) }}
							<!--  <input type="text" name="from_date" placeholder="DD/MM/YYYY" class="form-control" id="datepicker-8"  value="<?php echo $to_date; ?>" required readonly> -->
 ...
 {{ Form::text('to_date',$to_date,array('id'=>'datepicker-9','class'=>'form-control','placeholder'=>'DD/MM/YYYY','required'=>'required','readonly'=>'readonly')) }}
							<!--  <input type="text" name="to_date" placeholder="DD/MM/YYYY" id="datepicker-9" class="form-control" value="<?php echo $from_date; ?>" required readonly> -->
                               ...
                               <div class="col-sm-2">
								<a href="{{ url('').'/adm_payu_replacement_orders' }}">
...
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>{{ (Lang::has(Session::get('admin_lang_file').'.BACK_REPLACE_ORDERS')!= '')? trans(Session::get('admin_lang_file').'.BACK_REPLACE_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_REPLACE_ORDERS') }}</h5>
 ...
 @if(isset($_POST['submit']))
			...
			@foreach($replace_order_all as $allorders_list) 
				...
				@php	$orderstatus=((Lang::has(Session::get('admin_lang_file').'.BACK_SUCCESS')!= ''))? trans(Session::get('admin_lang_file').'.BACK_SUCCESS'): trans($ADMIN_OUR_LANGUAGE.'.BACK_SUCCESS');
					...
					@elseif($allorders_list->order_paytype==1)
				...
				@if($allorders_list->delivery_status == 9) 
					<td class="center">@if($allorders_list->order_id==$allorders_list->paypal_order_id)
					   @if($allorders_list->replace_status==1)<a href="javascript:void(0);"  class="btn btn-info" onclick="show_replace_approved('<?php echo $allorders_list->delStatus_id;?>');">Replace Request</a> @endif @endif
					</td>@elseif($allorders_list->delivery_status == 10)  @if($allorders_list->order_id==$allorders_list->paypal_order_id) @if($allorders_list->replace_status==2) 
					<td class="center">
						<a href="javascript:void(0);"  class="btn btn-info" onclick="show_replace_approved('<?php echo $allorders_list->delStatus_id;?>');">Replace Request Processed</a>
					</td> 
					@else <td>Order Replaced</td>
					@endif @else <td>Order Replaced</td>
				@endif @endif
	...
	<!-- Modal content-->
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title"> {{ (Lang::has(Session::get('admin_lang_file').'.MER_APPROVE_REPLACEMENT')!= '') ?  trans(Session::get('admin_lang_file').'.MER_APPROVE_REPLACEMENT') : trans($ADMIN_OUR_LANGUAGE.'.MER_APPROVE_REPLACEMENT') }}</h4>
			</div>
...
function show_replace_approved(data)
{
	...
	$("#approve_replace").load('<?php echo url(''); ?>/adm_get_approve_replacecontent_payu/'+data);
}
</script>
</body>
     <!-- END BODY -->
