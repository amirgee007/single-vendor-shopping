...
{!!$adminleftmenus!!}
                ...
                <div class="inner">
                    ...
                    <div class="col-lg-12">
                        	<ul class="breadcrumb">
                                ...
                                <li class="active"><a >{{ (Lang::has(Session::get('admin_lang_file').'.BACK_COD_HOLD_ORDERS')!= '') ? trans(Session::get('admin_lang_file').'.BACK_COD_HOLD_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_COD_HOLD_ORDERS') }}</a></li>
                            </ul>
                    </div>
                </div>
		...
		<!--  <form  action="{!!action('TransactionController@cod_hold_orders')!!}" method="POST"> -->
		 	{{ Form::open(array('action'=>'TransactionController@cod_hold_orders','method'=>'POST')) }}
 ...
 {{ Form::text('to_date',$to_date,array('id'=>'datepicker-9','class'=>'form-control','placeholder'=>'DD/MM/YYYY','required'=>'required','readonly'=>'readonly')) }}
							<!--  <input type="text" name="to_date" placeholder="DD/MM/YYYY" id="datepicker-9" class="form-control" value="{{$to_date}}" required readonly> -->
                             ...
                             <div class="col-sm-2">
							    <a href="{{ url('').'/cod_hold_orders' }}"><button type="button" name="reset" class="btn btn-block btn-info">@if (Lang::has(Session::get('admin_lang_file').'.BACK_RESET')!= '') {{  trans(Session::get('admin_lang_file').'.BACK_RESET') }} @else {{ trans($ADMIN_OUR_LANGUAGE.'.BACK_RESET') }} @endif</button></a>
							 </div>
							</div>
...
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>{{ (Lang::has(Session::get('admin_lang_file').'.BACK_COD_HOLD_ORDERS')!= '') ? trans(Session::get('admin_lang_file').'.BACK_COD_HOLD_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_COD_HOLD_ORDERS') }}</h5>
