...
<head>
    <meta charset="UTF-8" />
    <title>{{ $SITENAME }} {{ (Lang::has(Session::get('admin_lang_file').'.BACK_RETURN_ORDERS')!= '') ?  trans(Session::get('admin_lang_file').'.BACK_RETURN_ORDERS') : trans($ADMIN_OUR_LANGUAGE.'.BACK_RETURN_ORDERS') }}</title>
...
{!!$merchantleftmenus!!}
        ...
        <div id="content">
		 ...
		 <!-- <form  action="{!!action('TransactionController@adm_cod_return_orders')!!}" method="POST"> -->
		 	{{ Form::open(array('action'=>'TransactionController@adm_cod_return_orders','method'=>'POST')) }}
...
placeholder="DD/MM/YYYY"  id="datepicker-9" class="form-control" value="<?php echo $to_date; ?>" required readonly> -->
                               ...
                               <div class="col-sm-2">
								<a href="{{ url('').'/adm_cod_return_orders' }}">
...
function show_Return_approved(data)
{
	...
	$("#approve_Return").load('<?php echo url(''); ?>/adm_get_approve_REturncontent/'+data);
}
</script>
</body>
     <!-- END BODY -->
