...
<head>
    <meta charset="UTF-8" />
    <title><?php echo $SITENAME; ?> |Products Completed Orders</title>
    ...
    <div class="box dark">
		...
		<th class="sorting_asc" tabindex="0" aria-controls="dataTables-example" rowspan="1" colspan="1" style="width: 69px;" aria-label="S.No: activate to sort column ascending" aria-sort="ascending">S.No</th>
					...
					if($allorders_list->order_paytype==1)
					{
						...
						$ordertype = ((Lang::has(Session::get('admin_lang_file').'.BACK_PAYPAL')!= ''))? trans(Session::get('admin_lang_file').'.BACK_PAYPAL') : trans($ADMIN_OUR_LANGUAGE.'.BACK_PAYPAL');
					}
					else if($allorders_list->order_paytype==2)
					{
						$ordertype = ((Lang::has(Session::get('admin_lang_file').'.BACK_WALLET')!= ''))? trans(Session::get('admin_lang_file').'.BACK_WALLET') : trans($ADMIN_OUR_LANGUAGE.'.BACK_WALLET');	
					}else { $ordertype = "--"; }
					...
					<tr class="gradeA odd">
											...
											<ul>
											<?php
											$product_data = DB::table('nm_order')->orderBy('order_date', 'desc')->leftjoin('nm_product', 'nm_order.order_pro_id', '=', 'nm_product.pro_id')->where('nm_order.transaction_id', '=',$allorders_list->transaction_id)->get();
                                            ...
                                            <td class="center     "><?php echo $total_tax;?></td>
