...
<table border="0" cellspacing="0" cellpadding="0" width="600" align="center" style="border:1px solid #cccccc;">
  ...
  <tr bgcolor="#f5f5f5">  
        ...
        @endif 
          <td align="center"><img src="{{ $logo}}" alt="{{ $SITENAME }}"  style="margin:20px 0px; background:#f5f5f5; font-family: Montserrat"/></td>
  </tr>
  ...
  <tr>
      <td>
          <table width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;"> <tbody><tr> <td align="left" valign="top" style="color:#2c2c2c;display:block;line-height:20px;font-weight:300;margin:0 auto;clear:both;padding:20px" > <p style="padding:0;margin:0;font-size:16px;font-weight:bold;font-size:15px"> @if (Lang::has(Session::get('lang_file').'.HI')!= '') {{ trans(Session::get('lang_file').'.HI') }} @else {{ trans($OUR_LANGUAGE.'.HI') }} @endif {{ $customer_name }}, </p><br> <p style="padding:0;margin:0;color:#565656;font-size:13px"> @if (Lang::has(Session::get('lang_file').'.YOUR_REPLACE_REQUEST_IS_PROCESSED_PLEASE_FIND_THE_DETAILS')!= '') {{  trans(Session::get('lang_file').'.YOUR_REPLACE_REQUEST_IS_PROCESSED_PLEASE_FIND_THE_DETAILS') }} @else {{ trans($OUR_LANGUAGE.'.YOUR_REPLACE_REQUEST_IS_PROCESSED_PLEASE_FIND_THE_DETAILS') }} @endif!</p><br> <p style="padding:0;margin:0;color:#565656;line-height:22px;font-size:13px">  
            </td> </tr> </tbody></table>
      </td>
  </tr>
  <tr>
...
<tr>
  <td>
      ...
      <table width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;"> <tbody><tr> <td align="left" valign="top" style="color:#2c2c2c;display:block;line-height:20px;font-weight:300;margin:0 auto;clear:both;border-bottom:1px solid #e6e6e6;background-color:#f9f9f9;padding:20px" bgcolor="#F9F9F9"> <br> 
  ...
  <tr bgcolor="#f5f5f5">
    <td style="height:50px;text-align:center; font-size:14px; background:#f5f5f5;"><a href="<?php echo url('');?>" target="_blank"  style="text-decoration:none;color:#333333;font-weight:800;"> &copy; <?php echo date("Y"); ?>&nbsp; {{ $SITENAME }} </a></td>
  </tr>
