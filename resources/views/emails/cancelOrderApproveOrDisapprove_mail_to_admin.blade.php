...
<table border="0" cellspacing="0" cellpadding="0" width="600" align="center" style="border:1px solid #cccccc; font-family: Montserrat">
  ...
  <tr bgcolor="#f5f5f5"> 
        @php 
        $logo = public_path().'/public/assets/default_image/Logo.png'; 
  ...
  <tr>
     <td style="height:30px; text-align:center; font-weight:bold; font-size:15px;"><b>@if (Lang::has(Session::get('lang_file').'.ORDER_CANCELLATION_REQUEST')!= '') {{  trans(Session::get('lang_file').'.ORDER_CANCELLATION_REQUEST') }} @else {{ trans($MER_OUR_LANGUAGE.'.ORDER_CANCELLATION_REQUEST') }} @endif</b></td>
  </tr>
  <tr>
      <td>
  ...
  <tr>
     <td style=" margin:0 auto; font-size:15px;text-align:left; padding:10px 10px 10px; background: #f5f5f5;">
        <table  cellspacing="10">
          ...
          <tr>
            <th><?php if (Lang::has(Session::get('lang_file').'.PRODUCT_TITLE')!= '') { echo  trans(Session::get('lang_file').'.PRODUCT_TITLE');}  else { echo trans($MER_OUR_LANGUAGE.'.PRODUCT_TITLE');} ?></th>     
