...
<table border="0" cellspacing="0" cellpadding="0" width="600" align="center" style="border:1px solid #cccccc;font-family: Montserrat;">
  ...
  <tr bgcolor="#f5f5f5"> 
        ...
        $logo = $SITE_LOGO; @endphp
        @if(file_exists($SITE_LOGO))
        @php  $logo = $SITE_LOGO; @endphp
        @endif 
          <td align="center"><img src="{{ $SITE_LOGO }}" alt="{{ $SITENAME }}" style="margin:20px 0px;"/></td>
  </tr>
  <tr>
  ...
  <tr>
     <td style=" margin:0 auto; font-size:16px;text-align:left;padding:10px 10px 10px; background:#f5f5f5;">
        <table  cellspacing="10">
          ...
          <tr>     
          <th>@if (Lang::has(Session::get('lang_file').'.REASON_TO_RETURN')!= '') {{  trans(Session::get('lang_file').'.REASON_TO_RETURN') }} @else {{ trans($OUR_LANGUAGE.'.REASON_TO_RETURN') }} @endif</th>
             <th>:</th>
              <td>{{ $return_notes }}</td>
            </tr>
          <tr>     
...
<tr>
  <td>
      ...
      <table width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;"> <tbody><tr> <td align="left" valign="top" style="color:#333333;display:block;line-height:20px;font-weight:300;margin:0 auto;clear:both;padding:20px"> <br> 
  ...
  <tr bgcolor="#f5f5f5">
    <td style="height:50px;text-align:center; font-size:14px;"><a href="<?php echo url('');?>" target="_blank"  style="text-decoration:none;color:#333333;font-weight:800;"> &copy; <?php echo date("Y"); ?>&nbsp; {{ $SITENAME }} </a></td>
  </tr>
