...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="background:#fff;border-left: 3px solid #BF463D;border-right: 3px solid #BF463D;" class="hide">
    ...
    font-size: 20px;
    color: #000000;"> <?php if (Lang::has(Session::get('lang_file').'.HI')!= '') { echo  trans(Session::get('lang_file').'.HI');}  else { echo trans($OUR_LANGUAGE.'.HI');} ?> {{ $cus_name}}, </p><br>  <p style="padding:0;margin:0;color: #000000;
    line-height: 22px;
    font-size: 19px;
 ...
 <table width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;line-height:2.5;"> <tbody><tr> <td valign="top" align="center" style="background-color:#ffffff;display:block;margin:0 auto;clear:both;padding:5px 20px 0 20px" bgcolor="#fff"> <table border="0" cellspacing="0" cellpadding="0" width="100%" style="margin:0"> <tbody><tr> <td colspan="4" align="center" valign="top" style="padding:0 0 17px 0">  
   ...
   <span style="font-size:21px;color:#0266b5;font-weight:bold"><?php if (Lang::has(Session::get('lang_file').'.YOUR_LOGIN_CREDENTIALS_ARE')!= '') { echo  trans(Session::get('lang_file').'.YOUR_LOGIN_CREDENTIALS_ARE');}  else { echo trans($OUR_LANGUAGE.'.YOUR_LOGIN_CREDENTIALS_ARE');} ?>: </span><br> <span class="aBn" data-term="goog_231825889" tabindex="0"><span style="font-size:16px;font-weight:bold" class="aQJ"><?php if (Lang::has(Session::get('lang_file').'.USER_NAME')!= '') { echo  trans(Session::get('lang_file').'.USER_NAME');}  else { echo trans($OUR_LANGUAGE.'.USER_NAME');} ?> : {{ $first_name }}<br><?php if (Lang::has(Session::get('lang_file').'.PASSWORD')!= '') { echo  trans(Session::get('lang_file').'.PASSWORD');}  else { echo trans($OUR_LANGUAGE.'.PASSWORD');} ?> : {{ $password }}</span></span>  
  </p>  
    ...
    text-align: center;
    border: #ff9800 solid 1px;padding:8px 0;text-decoration:none;border-radius:2px;display:block;color:#fff;font-size:13px" href="<?php echo url(''); ?>/" align="center" target="_blank"> <span style="color:#ffffff;font-size:15px;font-weight:bold"><?php if (Lang::has(Session::get('lang_file').'.LOGIN_YOUR_ACCOUNT')!= '') { echo  trans(Session::get('lang_file').'.LOGIN_YOUR_ACCOUNT');}  else { echo trans($OUR_LANGUAGE.'.LOGIN_YOUR_ACCOUNT');} ?></span> </a> </p> </td> </tr> </tbody></table>
