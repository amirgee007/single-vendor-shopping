   ...
   <img src="{{ url('') }}/public/assets/themes/images/mail.jpg" class="hide-mob subsc-img" style="width:100% !important">
   	<div style="" class="subsc"><p>@if (Lang::has(Session::get('lang_file').'.WELCOME_TO_SUBSCRIBE_YOUR_NEWS_LETTER_SUBSCRIPTION')!= '') {{  trans(Session::get('lang_file').'.WELCOME_TO_SUBSCRIBE_YOUR_NEWS_LETTER_SUBSCRIPTION') }}  @else {{ trans($OUR_LANGUAGE.'.WELCOME_TO_SUBSCRIBE_YOUR_NEWS_LETTER_SUBSCRIPTION') }} @endif! {!! Form::open(array('url'=>'subscription_submit','class'=>'form-horizontal loginFrm')) !!}
		   ...
		   <div class="subsc-textbox" style="">
                        ...
                        <input type="email" id="email" name="email" placeholder="@if (Lang::has(Session::get('lang_file').'.ENTER_YOUR_MAIL_ID_FOR_EMAIL_SUBSCRIPTION')!= '') {{ trans(Session::get('lang_file').'.ENTER_YOUR_MAIL_ID_FOR_EMAIL_SUBSCRIPTION') }} @else {{ trans($OUR_LANGUAGE.'.ENTER_YOUR_MAIL_ID_FOR_EMAIL_SUBSCRIPTION') }} @endif" style=" padding: 3px 6px;" class="form-control span5 width-textsub" value="{!! Input::old('email') !!}" required>
                   		  
								
			  <input type="submit" id="register_submit" class="btn btn-warning" value="@if(Lang::has(Session::get('lang_file').'.SUBSCRIBE')!= '') {{  trans(Session::get('lang_file').'.SUBSCRIBE') }} @else {{ trans($OUR_LANGUAGE.'.SUBSCRIBE') }} @endif" style="
