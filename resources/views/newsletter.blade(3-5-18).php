...
<!-- Sidebar end=============================================== -->
    ...
    <ul class="breadcrumb">
		...
		<li class="active">@if (Lang::has(Session::get('lang_file').'.NEWS_LETTER')!= '') {{ trans(Session::get('lang_file').'.NEWS_LETTER') }} @else {{ trans($OUR_LANGUAGE.'.NEWS_LETTER') }} @endif</li>
    </ul>
	<h4 style="padding:10px;background:#eee;">@if (Lang::has(Session::get('lang_file').'.SUBSCRIBE_YOUR_MAIL_ID')!= '') {{ trans(Session::get('lang_file').'.SUBSCRIBE_YOUR_MAIL_ID') }} @else {{ trans($OUR_LANGUAGE.'.SUBSCRIBE_YOUR_MAIL_ID') }} @endif</h4>
    ...
    padding-top: 91px;
    position: absolute;
    width: 40%;" class="subsc">@if (Lang::has(Session::get('lang_file').'.WELCOME_TO_SUBSCRIBE_YOUR_NEWS_LETTER_SUBSCRIPTION')!= '') {{  trans(Session::get('lang_file').'.WELCOME_TO_SUBSCRIBE_YOUR_NEWS_LETTER_SUBSCRIPTION') }}  @else {{ trans($OUR_LANGUAGE.'.WELCOME_TO_SUBSCRIBE_YOUR_NEWS_LETTER_SUBSCRIPTION') }} @endif! {!! Form::open(array('url'=>'subscription_submit','class'=>'form-horizontal loginFrm')) !!}
    ...
    top: 560px;float:right;">
                        ...
                        <input type="email" id="email" name="email" placeholder="@if (Lang::has(Session::get('lang_file').'.ENTER_YOUR_MAIL_ID_FOR_EMAIL_SUBSCRIPTION')!= '') {{ trans(Session::get('lang_file').'.ENTER_YOUR_MAIL_ID_FOR_EMAIL_SUBSCRIPTION') }} @else {{ trans($OUR_LANGUAGE.'.ENTER_YOUR_MAIL_ID_FOR_EMAIL_SUBSCRIPTION') }} @endif" style="width:57%; padding: 3px 6px; margin-left:26px;" class="form-control span5 width-textsub" value="{!! Input::old('email') !!}" required>
                   		  
								
			  <input type="submit" id="register_submit" class="btn btn-warning" value="@if(Lang::has(Session::get('lang_file').'.SUBSCRIBE')!= '') {{  trans(Session::get('lang_file').'.SUBSCRIBE') }} @else {{ trans($OUR_LANGUAGE.'.SUBSCRIBE') }} @endif" style="margin-top: 20px;
    margin-left: 123px;
...
<div class="content">
   <div class="col-sm-12">
   <img src="{{ url('') }}/public/assets/themes/images/mail.jpg" class="hide-mob subsc-img" style="width:100% !important">
...
<script type="text/javascript" src="http://maps.googleapis.com/maps/api/js?sensor=false&libraries=places"></script>
   <script src="{{ url('') }}/themes/js/jquery-gmaps-latlon-picker.js"></script>
   ...
   <script type="text/javascript">
    ...
    google.maps.event.addDomListener(window, 'load', initialize);
		...
		$.ajaxSetup({
	        ...
	        <script src="http://demo.laravelecommerce.com/themes/js/jquery-1.10.0.min.js"></script>
